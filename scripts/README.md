...
| `record_demo.sh` | the video walkthrough as a real terminal session (for `asciinema rec -c`); prints `### t=N` markers the renderer keys captions to. `PAID=1` runs the real bracket (~$0.79), default is the free path. |
| `render_cast.py` | asciinema cast → captioned MP4 (`pyte` + Pillow + ffmpeg); `--caption-min` holds each caption for reading, `--max-idle` clamps dead air. |
